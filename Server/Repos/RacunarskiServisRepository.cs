...
{
    public class RacunarskiServisRepository
    {
        private readonly ProjectDbContext dbCtx;

        public RacunarskiServisRepository(ProjectDbContext ctx)
        {
            dbCtx = ctx;
        }
        ...
        public Common.Models.Racunarski_servis Get(int idServisa)
        {
            var servisFromDb = dbCtx.Racunarski_ServisSet.Find(idServisa);
            if (servisFromDb != null)
            {
                ...
                var servis = new Common.Models.Racunarski_servis()
                {
                    ID_servisa = servisFromDb.ID_servisa,
                    Naziv_serv = servisFromDb.Naziv_serv,
                    Tip_serv = (Common.Models.Tip_servisa)servisFromDb.Tip_serv,
                    Adresa_serv = adresa,
                    Br_tel_serv = brTel
                };
                return servis;
        ...
        public IEnumerable<Common.Models.Racunarski_servis> GetAll()
        {
            var retVal = new List<Common.Models.Racunarski_servis>();
            foreach (var servisFromDb in dbCtx.Racunarski_ServisSet.ToList())
            {
                var adresa = new Common.Models.Adresa(servisFromDb.Adresa_serv.Ulica, servisFromDb.Adresa_serv.Grad, servisFromDb.Adresa_serv.Broj, servisFromDb.Adresa_serv.PostanskiBroj);
                var brTel = new Common.Models.Broj_telefona(servisFromDb.Br_tel_serv.Pozivni_broj, servisFromDb.Br_tel_serv.Okrug, servisFromDb.Br_tel_serv.Broj);
                var servis = new Common.Models.Racunarski_servis() 
        ...
        public bool Delete(int idServisa)
        {
            try
            {
                dbCtx.Racunarski_ServisSet.Remove(dbCtx.Racunarski_ServisSet.FirstOrDefault((s) => s.ID_servisa == idServisa));
                dbCtx.SaveChanges();
                return true;
            }
            catch (Exception e)
